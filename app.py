...
def add_region():
    ...
    if request.method == 'POST':
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            c.execute('''
                INSERT INTO regions 
                (large_okrug, district_name, last_name, first_name, middle_name, address, phone, birth_date, location)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (large_okrug, district_name, last_name, first_name, middle_name, address, phone, birth_date, location))
            conn.commit()
...
def edit_region(region_id):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if request.method == 'POST':
            ...
            c.execute('''
                UPDATE regions
                SET large_okrug = ?, district_name = ?, last_name = ?, first_name = ?, middle_name = ?,
                    address = ?, phone = ?, birth_date = ?, location = ?
                WHERE id = ?
            ''', (large_okrug, district_name, last_name, first_name, middle_name,
                  address, phone, birth_date, location, region_id))
            conn.commit()
            return redirect(url_for('regions'))

        # GET - отримуємо дані для заповнення форми
    ...
    return render_template('edit_region.html', data=data)
...
def add_region1():
    ...
    if request.method == 'POST':
        ...
        c.execute('''
            INSERT INTO regions1 (okrug, district, last_name, first_name, middle_name,
...
@app.route('/regions2', methods=['GET'])
def region2():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
def add_region2():
    ...
    if request.method == 'POST':
        street = request.form['street']
        building = request.form['building']
        address_data = expand_buildings_2()
        district = address_data.get(street, {}).get('buildings', {}).get(building, '')

        c.execute('''
            INSERT INTO regions2 (okrug, district, last_name, first_name, middle_name,
                                   birth_date, street, building, apartment, phone, activist)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            2, district,
...
def add_region3():
    ...
    if request.method == 'POST':
        ...
        c.execute('''
            INSERT INTO regions3 (okrug, district, last_name, first_name, middle_name,
                                   birth_date, street, building, apartment, phone, activist)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            3, district,
